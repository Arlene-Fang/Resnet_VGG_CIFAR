...
def bn_relu_conv_pool_layer(input_layer, filter_shape, stride):
    '''
    A helper function to batch normalize, relu and conv the input layer sequentially
    :param input_layer: 4D tensor
    :param filter_shape: list. [filter_height, filter_width, filter_depth, filter_number]
    ...
    filter = create_variables(name='conv', shape=filter_shape)
    conv_layer = tf.nn.conv2d(relu_layer, filter, strides=[1, stride, stride, 1], padding='SAME')
    output = tf.layers.max_pooling2d(conv_layer, pool_size=(2, 2), strides=2)
    return conv_layer
...
def inference(input_tensor_batch, n, reuse):
    ...
    with tf.variable_scope('conv0', reuse=reuse):
        conv0 = bn_relu_conv_pool_layer(input_tensor_batch, [7, 7, 3, 64], 1)
