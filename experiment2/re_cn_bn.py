...
def relu_conv_bn_layer(input_layer, filter_shape, stride):
    '''
    A helper function to conv, batch normalize and relu the input tensor sequentially
    :param input_layer: 4D tensor
    :param filter_shape: list. [filter_height, filter_width, filter_depth, filter_number]
...
def relu_conv_bn_pool_layer(input_layer, filter_shape, stride):
    '''
    A helper function to conv, batch normalize and relu the input tensor sequentially
    :param input_layer: 4D tensor
    :param filter_shape: list. [filter_height, filter_width, filter_depth, filter_number]
    ...
    output = tf.layers.max_pooling2d(bn_layer, pool_size=(2, 2), strides=2)
    return output
...
def residual_block(input_layer, output_channel, first_block=False):
    ...
    with tf.variable_scope('conv1_in_block'):
        if first_block:
            ...
            conv1 = relu_conv_bn_layer(input_layer, [3, 3, input_channel, output_channel], stride)

    with tf.variable_scope('conv2_in_block'):
        conv2 = relu_conv_bn_layer(conv1, [3, 3, output_channel, output_channel], 1)
...
def inference(input_tensor_batch, n, reuse):
    ...
    with tf.variable_scope('conv0', reuse=reuse):
        conv0 = relu_conv_bn_pool_layer(input_tensor_batch, [7, 7, 3, 64], 1)
