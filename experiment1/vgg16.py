...
def conv_bn_relu_pool_layer(input_layer, filter_shape, stride):
    '''
    A helper function to conv, batch normalize and relu the input tensor sequentially
    :param input_layer: 4D tensor
    :param filter_shape: list. [filter_height, filter_width, filter_depth, filter_number]
    ...
    output = tf.nn.relu(bn_layer)
    output = tf.layers.max_pooling2d(output, pool_size=(2, 2), strides=2)
    return output
...
def inference(input_tensor_batch, n, reuse):
    ...
    with tf.variable_scope('conv1', reuse=reuse):
        conv1 = conv_bn_relu_pool_layer(layers[-1], [3, 3, 64, 64], 1)
        activation_summary(conv1)
        layers.append(conv1)

    with tf.variable_scope('conv2', reuse=reuse):
        conv2 = conv_bn_relu_layer(layers[-1], [3, 3, 64, 128], 1)
        activation_summary(conv2)
        layers.append(conv2)    

    with tf.variable_scope('conv3', reuse=reuse):
        conv3 = conv_bn_relu_pool_layer(layers[-1], [3, 3, 128, 128], 1)
        activation_summary(conv3)
        layers.append(conv3)

    with tf.variable_scope('conv4', reuse=reuse):
        conv4 = conv_bn_relu_layer(layers[-1], [3, 3, 128, 256], 1)
        activation_summary(conv4)
        layers.append(conv4)

    with tf.variable_scope('conv5', reuse=reuse):
        conv5 = conv_bn_relu_layer(layers[-1], [3, 3, 256, 256], 1)
        layers.append(conv5)

    with tf.variable_scope('conv6', reuse=reuse):
        conv6 = conv_bn_relu_layer(layers[-1], [3, 3, 256, 256], 1)
        activation_summary(conv6)
        layers.append(conv6)

    with tf.variable_scope('conv7', reuse=reuse):
        conv7 = conv_bn_relu_pool_layer(layers[-1], [3, 3, 256, 256], 1)
        layers.append(conv7)

    with tf.variable_scope('conv8', reuse=reuse):
        conv8 = conv_bn_relu_layer(layers[-1], [3, 3, 256, 512], 1)
        layers.append(conv8)        

    with tf.variable_scope('conv9', reuse=reuse):
        conv9 = conv_bn_relu_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv9)

    with tf.variable_scope('conv10', reuse=reuse):
        conv10 = conv_bn_relu_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv10)

    with tf.variable_scope('conv11', reuse=reuse):
        conv11 = conv_bn_relu_pool_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv11)

    with tf.variable_scope('conv12', reuse=reuse):
        conv12 = conv_bn_relu_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv12)

    with tf.variable_scope('conv13', reuse=reuse):
        conv13 = conv_bn_relu_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv13)

    with tf.variable_scope('conv14', reuse=reuse):
        conv14 = conv_bn_relu_layer(layers[-1], [3, 3, 512, 512], 1)
        layers.append(conv14)

    with tf.variable_scope('conv15', reuse=reuse):
        conv15 = conv_bn_relu_pool_layer(layers[-1], [3, 3, 512, 512], 1)
