from res34_use import *
from datetime import datetime
import time
from res_input import *
...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        loss, self.train_acc = self.loss(logits, self.label_placeholder)
        ...
        # Validation loss
        self.vali_loss, self.vali_acc = self.loss(vali_logits, self.vali_label_placeholder)
        ...
        self.train_op, self.train_ema_op = self.train_operation(global_step, self.full_loss,
                                                                self.train_top1_error, self.train_acc)
        self.val_op = self.validation_op(validation_step, self.vali_top1_error, self.vali_loss, self.vali_acc)
        ...
        for step in range(FLAGS.train_steps):
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if FLAGS.is_full_validation is True:
                    validation_loss_value, validation_error_value, validation_acc_value = self.full_validation(loss=self.vali_loss,
                                            top1_error=self.vali_top1_error, vali_data=vali_data,
                                            vali_labels=vali_labels, session=sess,
                                            batch_data=train_batch_data, batch_label=train_batch_labels, accuracy=self.vali_acc)
                    ...
                    _, validation_error_value, validation_loss_value, validation_acc_value = sess.run([self.val_op,
                                                                     self.vali_top1_error,
                                                                 self.vali_loss, self.vali_acc],
                ...
                val_acc_list.append(validation_acc_value)

            start_time = time.time()

            _, _, train_loss_value, train_error_value, train_acc_value = sess.run([self.train_op, self.train_ema_op,
                                                           self.full_loss, self.train_top1_error, self.train_acc],
                                {self.image_placeholder: train_batch_data,
                                  self.label_placeholder: train_batch_labels,
                                  self.vali_image_placeholder: validation_batch_data,
                                  self.vali_label_placeholder: validation_batch_labels,
            ...
            if step % FLAGS.report_freq == 0:
                ...
                print('Validation accuracy = %.4f' % validation_acc_value)
    ...
    def generate_vali_batch(self, vali_data, vali_label, vali_batch_size):
        ...
        '''
        offset = np.random.choice(7500 - vali_batch_size, 1)[0]
    ...
    def full_validation(self, loss, top1_error, session, vali_data, vali_labels, batch_data,
        ...
        for step in range(num_batches):
            ...
            feed_dict = {self.image_placeholder: batch_data, self.label_placeholder: batch_label,
                ...
                self.lr_placeholder: FLAGS.init_lr}
            loss_value, top1_error_value = session.run([loss, top1_error, accuracy], feed_dict=feed_dict)
            loss_list.append(loss_value)
            error_list.append(top1_error_value)
            acc_list.append(accuracy)
        return np.mean(loss_list), np.mean(error_list), np.mean(acc_list)
